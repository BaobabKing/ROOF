...
function [imgDemixed, unmixing_log] = PICASSO_4C(input_image, qN, maxIter, step_size, monitor)


imgChannel1 = input_image(:,:,1);
imgChannel2 = input_image(:,:,2);
imgChannel3 = input_image(:,:,3);
imgChannel4 = input_image(:,:,4);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
binF = 16;   % pixel binning
qN = 100;    % quantization
...
unmixing_log.mi14 = zeros(1, maxIter);
unmixing_log.mi22 = zeros(1, maxIter);
unmixing_log.mi23 = zeros(1, maxIter);
unmixing_log.mi24 = zeros(1, maxIter);
unmixing_log.mi33 = zeros(1, maxIter);
unmixing_log.mi34 = zeros(1, maxIter);
unmixing_log.mi44 = zeros(1, maxIter);


unmixing_log.nr1 = zeros(1, maxIter);
unmixing_log.nr2 = zeros(1, maxIter);
unmixing_log.nr3 = zeros(1, maxIter);
unmixing_log.nr4 = zeros(1, maxIter);

unmixing_log.max_alpha = zeros(1, maxIter);
unmixing_log.alpha_matrix = zeros(4, 4, maxIter);
...
for it=1:maxIter
    ...
    alpha3 = estimate_alpha(v1_img, v4_img, binF, qN); % y
    alpha_matrix(1,2) =  - alpha1;
    alpha_matrix(1,3) =  - alpha2;
    alpha_matrix(1,4) =  - alpha3; % y
    
    %% part 2
    alpha1 = estimate_alpha(v2_img, v1_img, binF, qN);
    alpha2 = estimate_alpha(v2_img, v3_img, binF, qN);
    alpha3 = estimate_alpha(v2_img, v4_img, binF, qN); % y
    alpha_matrix(2,1) =  - alpha1;
    alpha_matrix(2,3) =  - alpha2;
    alpha_matrix(2,4) =  - alpha3; % y
    
        
    %% part 3  
    alpha1 = estimate_alpha(v3_img, v1_img, binF, qN);
    alpha2 = estimate_alpha(v3_img, v2_img, binF, qN);
    alpha3 = estimate_alpha(v3_img, v4_img, binF, qN); % Y
    alpha_matrix(3,1) = - alpha1;
    alpha_matrix(3,2) = - alpha2;
    alpha_matrix(3,4) = - alpha3;

    %% part 4  
    alpha1 = estimate_alpha(v4_img, v1_img, binF, qN); % y
    alpha2 = estimate_alpha(v4_img, v2_img, binF, qN); % Y
    alpha3 = estimate_alpha(v4_img, v3_img, binF, qN); % Y
    alpha_matrix(4,1) = - alpha1; % y
    alpha_matrix(4,2) = - alpha2; % Y
    alpha_matrix(4,3) = - alpha3; % Y
    
    %% construct incremental unmixing matrix   
%    cUMM = eye(3) + step_size*alpha_matrix;
    cUMM = eye(4) + step_size*alpha_matrix; % Y
    UMM = cUMM*UMM;

    
    %% simuletaneous update    
    v1_img = cUMM(1,1)*v1_img + cUMM(1,2)*v2_img + cUMM(1,3)*v3_img + cUMM(1,4)*v4_img;
    v2_img = cUMM(2,1)*v1_img + cUMM(2,2)*v2_img + cUMM(2,3)*v3_img + cUMM(2,4)*v4_img;
    v3_img = cUMM(3,1)*v1_img + cUMM(3,2)*v2_img + cUMM(3,3)*v3_img + cUMM(3,4)*v4_img;   
    v4_img = cUMM(4,1)*v1_img + cUMM(4,2)*v2_img + cUMM(4,3)*v3_img + cUMM(4,4)*v4_img;   
    ...
    unmixing_log.nr4(it) = checkNegativity(monitor_image(:,:,4) ); % y
...
imgDemixed(:,:,4) = imgChannel4orth;  % y
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
